...
データは各端末のブラウザに保存され、アカウントを作成してサインインすることで複数端末間のクラウド同期が可能です。
...
- 設定により、日ごとの記録件数と記録時刻の分布を表示

### データ管理

- 指定した期間のメモを Markdown 形式でクリップボードにコピー
- 全件データを JSON または Markdown 形式でエクスポート
- JSON 形式のデータを既存データへマージしてインポート

### アカウント管理

- メールアドレスとパスワードによるアカウント作成とサインイン
- アカウント作成時にメールアドレスを OTP 認証
- パスワードを忘れた場合に OTP 認証を通してパスワードを再設定
- サインイン中にパスワードを変更

### ローカルでの利用

- アカウントを作成せずに利用でき、記録はアカウントに紐づかない「匿名データ」として端末のブラウザ内にのみ保存
- サインイン中はオフライン状態であってもアカウントごとのユーザーデータとして端末のブラウザ内にメモを記録
- 端末のブラウザ内に保存された匿名データを削除
- アカウント作成またはサインイン時に、匿名データをサインイン先のユーザーデータへ移行

### クラウド同期

- 端末のブラウザ内のユーザーデータとクラウド上のデータを同期
- サインイン中の自動同期と、設定画面からの手動同期
- 端末のブラウザ内のユーザーデータをアカウントごとに分けて保存
- アカウント削除時に記録を匿名データへ移行し、クラウド上のデータを削除
...
- Authentication Keys > Publishable (sb_publishable\_\*\*\* など)
...
- [アカウント認証](docs/authentication.md)
...
- 保存データ、インポートファイル、エクスポートデータにはサイズ上限があります
- 添付ファイルや画像の保存には対応していません

### データの編集と検索

- 記録済みのメモを編集する機能には対応していません
- 過去のメモを検索する機能には対応していません
